...
public class RobotMap {
    ...
    public RobotMap(int n, int m) {
        validateArgmentMap(n, m);
        this.n = n;
        this.m = m;
        this.robots = new ArrayList<>(size =5);
    ...
    public RobotMap(int n, int m, int size) {
        ...
        this.m = m;
        this.size  = size;
        this.robots = new ArrayList<>(size);
    }
    ...
    public Robot createRobot(Point point) {
        ...
        robots.add(robot);
        if (robots.size() > size) {
            throw new IllegalStateException("Роботов должно быть не больше" + size);
        }
        return robot;
    }
    ...
    // Валидация карты
    private void validateArgmentMap(int n, int m) {
        if (n <= 0 || m <= 0) {
            throw new IllegalStateException("Неккоректное значение параметров карты");
        }
    }
